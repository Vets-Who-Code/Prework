<h1 align="center">
  <img src="./img/vwc.gif" alt="Vets Who Code" width="100px" /> <a href="https://vetswhocode.io">VETS WHO CODE</a>
</h1>
...
At #VetsWhoCode, we believe that long-form writing is one of the best ways to showcase your expertise and communicate effectively. It allows you to document important information, scale your knowledge, and advocate for yourself to non-technical employers. Since your Dev.to audience is primarily composed of tech professionals, writing can be a valuable skill to distinguish yourself and stand out in the industry.
...
Now that we have made a new directory lets go there where we will put our work on our machine. In both windows and MacOS we do this by typing `cd` and then pressing enter:
...
An IDE can also provide features like auto-completion, syntax highlighting, and code refactoring to make coding faster and more efficient. IDEs are used for developing software applications, web applications, and mobile apps. Our preferred IDE is Visual Studio Code. Download it [here](https://code.visualstudio.com/). You can also install our developer extension pack by going to extensions and typing [VetsWhoCode Extension Pack](https://marketplace.visualstudio.com/items?itemName=VetsWhoCode.vetswhocode-extension-pack).
...
5. Start coding: Once you have cloned the repository, you can change the code using your favorite code editor. To do this, open the repository you just downloaded with VS Code (it's just a folder). Make text changes in files, create/remove folders and update their names. You're now coding.
...
8. Push changes: To upload your changes to the GitHub repository, type `git push`. If your branch isn't setup, check the output in the command line. It may have a message like `git push --set-upstream origin <branch name>`. Once you've set your upstream, you can use `git push` for all future uploads to this branch.
...
1. Set up your environment: To create an HTML and CSS document, you'll need a text editor and a web browser. You can use any text editor, such as Notepad, Sublime Text, or Atom. You can also use online editors like [CodePen](https://codepen.io/) or [JSFiddle](https://jsfiddle.net/) to get started quickly.
...
- **Variables**: You can declare variables using the `var`, `let`, or `const` keywords. Variables hold values that can be used later in your code. While var and let can have interchangeable values, the value in `const` is not interchangeable. Examples:
